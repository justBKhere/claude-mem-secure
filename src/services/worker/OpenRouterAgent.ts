...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      const { apiKey, model, siteUrl, appName } = await this.getOpenRouterConfig();

      if (!apiKey) {
        throw new Error('OpenRouter API key not configured. Set OPENROUTER_API_KEY environment variable or store in OS keyring.');
      }
  ...
  /**
   * Get OpenRouter configuration from keyring, settings, or environment
   */
  private async getOpenRouterConfig(): Promise<{ apiKey: string; model: string; siteUrl?: string; appName?: string }> {
    const settingsPath = USER_SETTINGS_PATH;
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);

    // API key: try keyring first, then environment variable (no longer checking settings.json)
    let apiKey = '';
    try {
      const keyringValue = await keyringManager.getSecret('OPENROUTER_API_KEY');
      if (keyringValue) {
        apiKey = keyringValue;
        logger.debug('SDK', 'OpenRouter API key retrieved from OS keyring');
      } else {
        // Fallback to environment variable
        const envValue = process.env.OPENROUTER_API_KEY;
        if (envValue) {
          apiKey = envValue;
          logger.debug('SDK', 'OpenRouter API key retrieved from environment variable');
        }
      }
    } catch (error) {
      logger.warn('SDK', 'Failed to retrieve OpenRouter API key from keyring, falling back to environment', {
        error: error instanceof Error ? error.message : String(error)
      });
...
export async function isOpenRouterAvailable(): Promise<boolean> {
  try {
    ...
    logger.warn('SDK', 'Failed to check OpenRouter availability from keyring, checking environment', {
      error: error instanceof Error ? error.message : String(error)
    });
    return !!process.env.OPENROUTER_API_KEY;
  }
