...
export class GeminiAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      const { apiKey, model, rateLimitingEnabled } = await this.getGeminiConfig();

      if (!apiKey) {
        throw new Error('Gemini API key not configured. Set GEMINI_API_KEY environment variable or store in OS keyring.');
      }
  ...
  /**
   * Get Gemini configuration from keyring, settings, or environment
   */
  private async getGeminiConfig(): Promise<{ apiKey: string; model: GeminiModel; rateLimitingEnabled: boolean }> {
    ...
    let apiKey = '';
    try {
      const keyringValue = await keyringManager.getSecret('GEMINI_API_KEY');
      if (keyringValue) {
        apiKey = keyringValue;
        logger.debug('SDK', 'Gemini API key retrieved from OS keyring');
      } else {
        // Fallback to environment variable
        const envValue = process.env.GEMINI_API_KEY;
        if (envValue) {
          apiKey = envValue;
          logger.debug('SDK', 'Gemini API key retrieved from environment variable');
        }
      }
    } catch (error) {
      logger.warn('SDK', 'Failed to retrieve Gemini API key from keyring, falling back to environment', {
        error: error instanceof Error ? error.message : String(error)
      });
...
export async function isGeminiAvailable(): Promise<boolean> {
  try {
    ...
    logger.warn('SDK', 'Failed to check Gemini availability from keyring, checking environment', {
      error: error instanceof Error ? error.message : String(error)
    });
    return !!process.env.GEMINI_API_KEY;
  }
