...
export class Server {
  ...
  private setupCoreRoutes(): void {
    ...
    this.app.get('/api/auth/token', requireLocalhost, async (_req: Request, res: Response) => {
      try {
        const token = await tokenAuth.getOrCreateToken();
        const maskedToken = await tokenAuth.getTokenForDisplay();
        const isFirstCall = !maskedToken || maskedToken === token.substring(0, 8) + '...';

        res.json({
          token: isFirstCall ? token : maskedToken,
          masked: !isFirstCall,
          warning: isFirstCall ? 'Save this token securely. It will only be shown in full once.' : undefined
        });
      } catch (error) {
        logger.error('AUTH', 'Failed to get auth token', {}, error as Error);
        res.status(500).json({ error: 'Failed to retrieve authentication token' });
      }
    });

    this.app.post('/api/auth/regenerate', requireLocalhost, requireAuth, async (_req: Request, res: Response) => {
      try {
        const newToken = await tokenAuth.regenerateToken();

        logger.info('AUTH', 'Authentication token regenerated');
        ...
        logger.error('AUTH', 'Failed to regenerate auth token', {}, error as Error);
        res.status(500).json({ error: 'Failed to regenerate authentication token' });
      }
    });

    // Admin endpoints for process management (localhost-only + auth required)
    this.app.post('/api/admin/restart', requireLocalhost, requireAuth, async (_req: Request, res: Response) => {
    ...
    this.app.post('/api/admin/shutdown', requireLocalhost, requireAuth, async (_req: Request, res: Response) => {
