...
import { initializeRetentionManager } from './retention/index.js';
...
export class WorkerService {
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      // Run retention cleanup in background (non-blocking)
      const retentionManager = initializeRetentionManager(this.dbManager.getSessionStore());
      retentionManager.runCleanup().catch(error => {
        logger.error('SYSTEM', 'Retention cleanup failed during startup', {}, error as Error);
      });
